...
namespace Memba.FileUpload.Providers
{
    ...
    public class SimpleFileStorageProvider : FileStorageProviderBase
    {
        private const string NAME = "SimpleFileStorageProvider";
        private const string DESCRIPTION = "Simple file system storage provider";
        ...
        public override object GetNewFileKey(string fileName)
        {
            ...
            //fileName is the path of the file on the client 
            FileInfo objFileInfo = new FileInfo(fileName);
            //So we get just the file name
            string sFileName = objFileInfo.Name;
            //and build a path to teh storage folder on the server
            string sFilePath = Path.Combine(_ConnectionString, sFileName);

            //We check whether the file already exists on the server
            ...
            //The dot in first or last position does not occur
            else
            {
                sFileNameWithoutExt = sFileName.Substring(0, iPos);
            ...
            do
	        {
                i++; //Actually start at 1

                //Search for a tilde in the file name
                iPos = sFileNameWithoutExt.LastIndexOf('~');

                string sFileNameWithoutTilde;
                if (iPos < 0) //No tilde
                    sFileNameWithoutTilde = sFileNameWithoutExt;
                else if (iPos == sFileNameWithoutExt.Length - 1) //A tilde at the end of the file name without extension
                    sFileNameWithoutTilde = sFileNameWithoutExt.Substring(0, iPos);
                else //A tilde within the file name without extension
                {
                    string sNum = sFileNameWithoutExt.Substring(iPos + 1);
                    int iNum;
                    bool bTry = Int32.TryParse(sNum, out iNum);
                    if (bTry) //A number follows the tilde
                    {
                        sFileNameWithoutTilde = sFileNameWithoutExt.Substring(0, iPos);
                    }
                    else //Other characters follow the tilde which is part of the name
                    {
                        sFileNameWithoutTilde = sFileNameWithoutExt;
                    }
                }

                string sNewFileName = sFileNameWithoutTilde + "~" + i.ToString() + "." + sExtension;
                objFileInfo = new FileInfo(Path.Combine(_ConnectionString, sNewFileName));


	        } while (objFileInfo.Exists);
        ...
        public override void DeleteFile(object providerFileKey)
        {
            ...
            //Note: let exceptions propagate up the stack
        }
        /// <summary>
        /// 
        /// </summary>
